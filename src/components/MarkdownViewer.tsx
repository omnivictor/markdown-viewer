...
import { useState, useEffect, useRef, useMemo, useCallback, memo, type ComponentProps } from 'react';
...
const remarkPlugins = [remarkGfm, remarkEmoji, remarkBreaks];
const rehypePlugins = [rehypeRaw, [rehypeSanitize, sanitizeSchema], rehypeHighlight, rehypeSlug] as any;

// Memoized ReactMarkdown to avoid re-parsing on unrelated re-renders
const MarkdownContent = memo(function MarkdownContent({
  ...
  const components = useMemo(() => ({
    ...
    input: ({ type, checked, ...props }: any) => {
      if (type === 'checkbox') {
        return <input type="checkbox" checked={checked} readOnly className="gh-checkbox mr-2" {...props} />;
      }
    ...
    <div className="h-full flex flex-col">
      ...
      >
        <div className="gh-preview-content p-6 text-sm leading-relaxed">
          <MarkdownContent content={debouncedContent} copiedBlock={copiedBlock} onCopy={handleCopy} />
        </div>
      </div>
    </div>
  );
